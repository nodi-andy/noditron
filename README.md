...
No registry server of ours is involved: a module is just a GitHub repo (public — tag it with the topic `noditron-module` to show up in the library dialog's own browsable list — or private), fetched through the GitHub Contents API, no publish step beyond pushing. Opening "Add from library…" loads that list straight away (the search box just filters it); a private repo needs a personal access token first, entered once via the "Import from a repo / manage GitHub token…" subdialog and kept only in this browser's `localStorage` — the same token nodigraph's own "Open/Save to GitHub" already uses (shared key, set it in either place), never sent anywhere but `api.github.com`. That same subdialog also covers a repo not yet showing up in the list (pin an owner/repo/ref/path directly). One important constraint of the browse list specifically: it always fetches `noditron.module.json` from a repo's **root** — one module per repo for that path; a manifest that isn't at the root (a monorepo of several modules, say) only installs through the subdialog's own path field. (An earlier version of this fetched public repos only, through jsDelivr's GitHub CDN — dropped once it turned out every repo in this project except `nodigraph` itself is private, which jsDelivr can never read regardless of a token.)
...
`noditron.module.json` at this repo's own root is a working example built this way: a blank ESP32 board with no ports yet (nothing is known about it until it's connected) and a real connect/flash dialog — see "Flashing firmware over serial" below. It's only actually discoverable through the browse list once this repo itself carries the `noditron-module` GitHub topic — a repo-settings change with no API this project's tools can reach, so add it by hand under Settings → General → Topics if you want it to show up there; the manual-import subdialog installs it either way.
