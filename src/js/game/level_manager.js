...
export class LevelManager {
    ...
    addEntity(className, x, y, val, val2, buildingVariant, toggleValue) {
        let newEntity;
        if (buildingVariant == undefined) buildingVariant = defaultBuildingVariant;
        newEntity = this.root.logic.tryPlaceBuilding({
            origin: new Vector(x, y),
            rotation: 0,
            rotationVariant: 0,
            originalRotation: 0,
            building: gMetaBuildingRegistry.findByClass(className),
            variant: buildingVariant,
        });

        if (newEntity.components.NodiButton) {
            newEntity.components.NodiButton.storedCount = val;
            if (toggleValue) newEntity.components.NodiButton.toggled = toggleValue;
            ...
            if (toggleValue) {
                newEntity.components.NodiLed.toggled = toggleValue;
                newEntity.components.NodiLed.initialToggled = toggleValue;
            }
    ...
    addWall(x, y) {
        ...
        if (Array.isArray(x)) {
            ...
            this.root.logic.tryPlaceBuilding({
                ...
                rotationVariant: 0,
                originalRotation: 0,
                building: gMetaBuildingRegistry.findByClass(MetaNodiOperBuilding),
    ...
    setupLevel(level) {
        if (level == 1) {
            ...
            this.addEntity(MetaNodiLedBuilding, 2, 2, 12, 12, defaultBuildingVariant, true);
        } else if (level == 5) {
            // 5 + 7 = 12
            this.addEntity(MetaNodiLedBuilding, 3, 3, 12, 12);
            this.addEntity(MetaNodiButtonBuilding, -5, 3, 5);
            this.addEntity(MetaNodiLedBuilding, 0, 0, 7, 7, defaultBuildingVariant, true);
        } else if (level == 6) {
            // ((1+1)+2)+4) = 8
            ...
            this.addEntity(MetaNodiLedBuilding, 2, 3, 5, 5, defaultBuildingVariant, true);
            this.addEntity(MetaNodiLedBuilding, -4, -2, 3, 3, defaultBuildingVariant, true);
            this.addEntity(MetaNodiLedBuilding, 0, -2, 2, 2, defaultBuildingVariant, true);
            this.addEntity(MetaNodiLedBuilding, 4, -2, 11, 11);
        } else if (level == 8) {
            // 1+1+1+1+1+1 = 6  or  3+3=6
            this.addEntity(MetaNodiButtonBuilding, -5, 1, 1);
            this.addEntity(MetaNodiLedBuilding, 0, -2, 3, 3, defaultBuildingVariant, true);
            ...
            this.addEntity(MetaNodiLedBuilding, -2, 2, 0, 0, defaultBuildingVariant, true);
            this.addEntity(MetaNodiLedBuilding, 3, 2, -5, -5, defaultBuildingVariant, false);
        } else if (level == 13) {
            // Main > 3+1+1+1 = 6
            this.addEntity(MetaNodiOperBuilding, -1, -2, 1, 0, enumNodiOperVariants.starter);
            this.addEntity(MetaNodiLedBuilding, -3, 2, 1, 1, defaultBuildingVariant, true);
            this.addEntity(MetaNodiLedBuilding, 4, 0, 1, 1, defaultBuildingVariant, true);
            this.addEntity(MetaNodiLedBuilding, 7, 0, 10, 10);
        } else if (level == 14) {
            // Main > -5 * -5 =25
            this.addEntity(MetaNodiOperBuilding, -1, -2, 1, 0, enumNodiOperVariants.starter);
            this.addEntity(MetaNodiLedBuilding, 3, 2, -5, -5, defaultBuildingVariant, true);
